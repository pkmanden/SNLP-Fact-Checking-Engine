...
/**
 *
 * Class to create the build the corpus. An object is created if the subject of
 * the fact cannot be found in the corpus already.
 *
 * Connection to the web source (Wikipedia),and extraction and formatting of the
 * information from the infobox is done by functions inside this class.
 *
 */
			...
			for (Element table : tables) {
				if (table.className().contains("infobox") == true) {
					...
					for (Element element : elements) {
						...
						for (String requiredCategory : CorpusHandler.categoryDefinition.keySet()) {
							if ((element.text().toLowerCase()).contains(requiredCategory.toLowerCase())) {
								...
								if (CorpusHandler.corpus.containsKey(mySubjectRaw)) {
									...
									if (innerMap.containsKey(requiredCategory)) {
										factString = innerMap.get(requiredCategory);
									}
								}
								factString.add(element.text());
								innerMap.put(requiredCategory, factString);
